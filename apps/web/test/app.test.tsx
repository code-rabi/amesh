...
describe("App shell", () => {
  ...
  it("creates a cross-node allow rule from visible agent connection buttons", async () => {
    ...
    await waitFor(
      () =>
        expect(
          screen.getByRole("button", { name: /start connection from openclaw/i }).textContent
        ).toContain("Link"),
      { timeout: 3000 }
    );
    fireEvent.click(screen.getByRole("button", { name: /start connection from openclaw/i }));
    const targetButton = screen.getByRole("button", { name: /connect openclaw to codex/i });
    expect(targetButton.textContent).toContain("Target");
