...
`amesh` is a distributed control plane for ACP-compatible coding agents. A central server maintains node registration, agent inventory, routing policy, chat sessions, and live event streams. Remote node daemons connect to the server over one persistent WebSocket tunnel, advertise locally available agents, and execute ACP traffic against those agents through `acpx`. The web UI lets an operator chat with any registered agent and define which agents may trigger other agents.
...
2. Register multiple ACP-backed agents per node from a small local capabilities file.
...
- maintains one persistent WebSocket connection to the control plane
- reads a small local JSON file describing which local agents are available
- advertises available local agents
- starts and manages local `acpx` sessions when instructed by the server
...
- `acpx` as the sole local ACP execution backend for MVP
...
- the daemon can invoke `acpx` as a subprocess without forcing installers or operators to know ACP details
- this keeps the daemon thin while still giving the product one-command bootstrap and remote start semantics
...
Two logical channels exist over one persistent node WebSocket:
...
5. Daemon advertises installed agent definitions from local JSON.
...
4. Server instructs the node to start or resume the requested agent session.
5. Node executes the request against the local ACP agent by spawning `acpx`.
6. Streamed ACP output is pushed back through the node tunnel to the UI.
...
4. If allowed, the server routes the invocation to the target node over its existing tunnel.
5. The target node starts or resumes Agent B locally through `acpx`.
...
### Keep The Node Tunnel Thin

The daemon should not grow a general plugin or adapter system in MVP. Its job is to read local capabilities, maintain one tunnel, start `acpx` on demand, and forward ACP traffic cleanly.
...
1. `acpx` is still early, so version pinning and subprocess lifecycle handling matter.
2. Agent-to-agent invocation semantics need a strict schema, or routing becomes hard to reason about.
3. Auto-discovery across different local CLIs may become inconsistent across hosts.
...
3. Should the UI expose raw ACP traffic from day one, or only selected chat events?
4. Should local capabilities stay fully file-based for MVP, or should the daemon also probe the host for known CLIs?
